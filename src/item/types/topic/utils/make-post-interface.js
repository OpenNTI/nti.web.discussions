...
class TopicPostInterface {
	constructor(topic) {
		if (!topic) {
			throw new Error(
				'Cannot create a TopicPostInterface without a topic'
			);
		}
	...
	get contextId() {
		return this.topic.ContainerId;
	}
	get contextTitle() {
		return this.topic.ContainerTitle;
	}
	...
	get CreatedTime() {
		return this.topic.getCreatedTime();
	}
	get LastModified() {
		return this.topic.getLastModified();
	}
	...
	get body() {
		return this.topic.headline && this.topic.headline.body;
	}

	get isPinned() {
		return this.topic.isPinned;
	}
	get isFlagged() {
		return this.topic.isFlagged;
	}
	...
	get canAddComment() {
		return this.topic.canAddComment();
	}
	get commentCount() {
		return this.topic.PostCount;
	}
	...
	async getMostRecentComments() {
		...
		const contents = await topic.getContents({
			batchSize: 2,
			sortOn: 'CreatedTime',
	...
	addCommentAddedListener(fn) {
		...
		return () =>
			Events.removeListener(Events.TOPIC_COMMENT_CREATED, handler);
	}
	...
	addCommentUpdatedListener(fn) {
		...
		return () =>
			Events.removeListener(Events.TOPIC_COMMENT_UPDATED, handler);
	}
	...
	addCommentDeletedListener(fn) {
		...
		return () =>
			Events.removeListener(Events.TOPIC_COMMENT_DELETED, handler);
	}
}
