...
import { DisplayName, Text, Flyout } from '@nti/web-commons';
...
export default function Pill({
	...
	if (onlyMe) {
		...
		label = unknown ? (
			<Text.Base>
				{t('multiple.other', { count: sharedWith.length })}
			</Text.Base>
		) : (
			<Text.Base>
				{t('multiple.users', { count: sharedWith.length })}
			</Text.Base>
		);
	...
	if (!multiple && !onRemove && sharedWith && !noLink) {
		pill = <LinkTo.Object object={sharedWith}>{pill}</LinkTo.Object>;
	}
