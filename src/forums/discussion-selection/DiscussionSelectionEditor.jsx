...
export default class DiscussionSelectionEditor extends React.Component {
	...
	onForumsLoaded(courseDiscussionTopics, forums) {
		...
		const isSimple =
			courseDiscussionTopics.length === 0 &&
			forums.length === 1 &&
			forums[0].title === 'Other Discussions';
		...
		this.setState(
			{
				breadcrumb: push(null, {
					...
					activate: () =>
						this.onForumsLoaded(
							this.state.courseDiscussionTopics,
							this.state.forums
						),
				}),
				step: STEPS.FORUM_LIST,
	...
	onForumSelect = (forum, isHidden) => {
		...
		if (sections?.length <= 1) {
			this.setState({ breadcrumb, selectedForum: forum }, () => {
				this.onSectionSelect(sections?.[0], true);
			});
		} else {
			this.setState({
				breadcrumb,
		...
		return (
			<div>
				...
				{step === STEPS.FORUM_LIST && (
					<Forums
						{...this.state}
						onForumSelect={this.onForumSelect}
						onSelect={this.onTopicSelect}
					/>
				)}

				{step === STEPS.SECTION_LIST && (
					<Sections {...this.state} onSelect={this.onSectionSelect} />
				)}

				{step === STEPS.BOARD_LIST && (
					<Boards {...this.state} onSelect={this.onBoardSelect} />
				)}

				{step === STEPS.TOPIC_LIST && (
					<Topics {...this.state} onSelect={this.onTopicSelect} />
				)}
			</div>
		);
	}
...
function Forums({
	...
	const filteredDiscussionTopics = filterItemsBySearchTerm(
		courseDiscussionTopics,
		searchTerm
	);
